...
const collectInfrastructures = (
  text: string[],
  result: string[],
  isRequiredInfrastructure: (w: string) => boolean,
  infrastructure: string,
) => {
...
const collectBinarInfrastructures = (
  text: string[],
  result: string[],
  isRequiredInfrastructure: (w1: string, w2: string) => boolean,
  infrastructure: string,
) => {
  for (let i = text.length - 1; i >= 0; i--) {
...
const collectMultipleProperties = (
  text: string[],
  start: number,
  end: number,
  properties: string[],
...
const parseStreetWithoutWordStreet = (text: string, result: string[]): string => {
  ...
  for (let i = arr.length - 1; i >= startIdx; i--) {
    if (booleanUtils.areBuildings(arr[i]) || booleanUtils.areHouses(arr[i])) {
      ...
      propPrevLength = properties.length - propPrevLength;
    }
  }
